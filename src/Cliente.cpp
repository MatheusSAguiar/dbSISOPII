...
Cliente::Cliente (string username, string ServidorAddr, int ServidorDistributorPort, string localIp) : username(username), syncDirPath("/tmp/sync_dir_"+username+"/")
{
	...
	get_sync_dir();
	this->listenToMaster = new FuncoesSocket(10000);
	thread listenToMasterThread(&Cliente::listenMaster, this);
...
void Cliente::listenMaster() {
	while(1) {
		Pacote * data = this->listenToMaster->receive(TIMEOUT_OFF);
		if (data->type == TYPE_NEW_BOSS) {
			
			while(1) {
				FuncoesSocket socketToGetPort(string(data->payload), 4000);
				Pacote request = make_packet(TYPE_MAKE_CONNECTION, 1, 1, this->localIp.length(), this->localIp.c_str(), this->username.c_str());
				socketToGetPort.send(&request);
				Pacote *newPort = socketToGetPort.receive(TIMEOUT_ON, 1500);
				if(newPort != NULL && newPort->type == TYPE_MAKE_CONNECTION){
				
					socketMtx.lock();
					this->socket = new FuncoesSocket(string(data->payload), stoi(newPort->payload));
